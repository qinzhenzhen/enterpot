...
!(function($) {
	var g = {
		...
		proDetail: function() {
			...
			var userId = JSON.parse(localStorage.getItem("user")) ? JSON.parse(localStorage.getItem("user")).id : "";
			$.ajax({
					type: 'post',
					url: indexUrl,
					data: {
				...
				.done(function(data) {
					...
					if($(".doing:has(li)").length == 0) {
						$(".doing").html("还没有正在抢的商品哦").css({
							"text-align": "center",
							"font-size": "0.24rem"
						});
					}
					if($(".all:has(li)").length == 0) {
						$(".doing").html("还没有商品哦").css({
							"text-align": "center",
							"font-size": "0.24rem"
						});
					}
					...
					$(".con span").each(function() {
						...
						if(charNumbers > limit) {
							...
							$(this).parent().find("a").click(function() {
								if(cur_status == "less") {
									$(this).siblings().height(contentHeight).html(orgText).animate({
										height: orgHeight
									}, {
										duration: "fast"
									});
									...
									$(this).siblings().height(orgHeight).html(showText).animate({
